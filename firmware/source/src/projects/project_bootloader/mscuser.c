...
//		SST25_Write(MSC_SECTOR_SIZE + MSC_FLASH_START,"thienhaiblue",12);
//		SST25_Read(MSC_SECTOR_SIZE + MSC_FLASH_START,ttbuf,12);
		printf_dbg("G:%d:%d:%d:%d\r\n",current_sector,number_of_sector_transfer,index_of_data_transfer,BulkLen);
		SST25_Read(current_sector*MSC_SECTOR_SIZE + MSC_FLASH_START + index_of_data_transfer,buff_data,MSC_MAX_PACKET);
		USB_WriteEP(MSC_EP_IN, buff_data,MSC_MAX_PACKET);
		index_of_data_transfer += MSC_MAX_PACKET;
		if(index_of_data_transfer >= MSC_SECTOR_SIZE)
...
void MSC_MemoryWrite (void) {
	uint32_t n,i = 10000,writeFlag = 0,j;
  ...
  if (current_sector >= MSC_NUM_SECTOR_MAX) {
	...
	if(BulkLen != MSC_MAX_PACKET)
		asdf++;
	printf_dbg("S:%d:%d:%d:%d:%d\r\n",current_sector,SST25_sectorOffset,number_of_sector_transfer,index_of_data_transfer,dataOffset);
  ...
  if(index_of_data_transfer >= MSC_SECTOR_SIZE)
  {
		if(dataOffset + index_of_data_transfer >= SECTOR_SIZE)
		{
			writeFlag = 1;
		}
		number_of_sector_transfer--;
