...
//			}
			...
			)
			{
				...
				if(timeGetGPS >= 10)
				{
					...
					fTemp = formatLat(nmeaInfo.lat,nmeaInfo.ns);
					gpsLat.value = DB_FloatToU32(fTemp);
					fTemp = formatLng(nmeaInfo.lon,nmeaInfo.ew);
					gpsLon.value = DB_FloatToU32(fTemp);
					gpsHDOP.value = DB_FloatToU32(nmeaInfo.HDOP);
					gpsDir.value = DB_FloatToU32(nmeaInfo.direction);
					
					cellLocate.lat = nmeaInfo.lat;
					cellLocate.lon = nmeaInfo.lon;
					cellLocate.ew = nmeaInfo.ew;
					cellLocate.ns = nmeaInfo.ns;
					cellLocate.alt = nmeaInfo.HDOP;
					cellLocate.dir = nmeaInfo.direction;
					cellLocate.speed = nmeaInfo.speed;
