...
{
	HAL_SPI_ACCEL_CS_ASSERT;
	halSpiWriteByte(XL362_REG_READ);
	halSpiWriteByte(regaddr);
	while(count--)
		*buf++ = halSpiWriteByte(0xff);
...
{
	HAL_SPI_ACCEL_CS_ASSERT;
	halSpiWriteByte(XL362_FIFO_READ);
	while(count--)
		*buf++ = halSpiWriteByte(0xff);
	HAL_SPI_ACCEL_CS_DEASSERT;
}
...
{
	HAL_SPI_ACCEL_CS_ASSERT;
	halSpiWriteByte(XL362_REG_WRITE);
	halSpiWriteByte(regaddr);
	while(count)
	{
		halSpiWriteByte(*buf++);
